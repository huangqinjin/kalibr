#ifndef ASLAM_CAMERAS_PERSPECTIVE_DISTORTION_HPP
#define ASLAM_CAMERAS_PERSPECTIVE_DISTORTION_HPP
...
class PerspectiveDistortion {
 public:
  ...
  /// c = 4 5 8 12 14
  explicit PerspectiveDistortion(int c);

  explicit PerspectiveDistortion(const sm::PropertyTree & config);

  virtual ~PerspectiveDistortion();
  ...
  bool isBinaryEqual(const PerspectiveDistortion & rhs) const;

  static PerspectiveDistortion getTestDistortion();
...
#include "implementation/PerspectiveDistortion.hpp"

SM_BOOST_CLASS_VERSION (aslam::cameras::PerspectiveDistortion);

#endif /* ASLAM_CAMERAS_PERSPECTIVE_DISTORTION_HPP */
