...
class IrscCameraChain():
    def __init__(self, chainConfig, targetConfig, parsed):
        ...
        for camNr in range(0, chainConfig.numCameras()):
            ...
            #create the camera
            self.camList.append( IrscCamera( camConfig, 
                                            targetConfig, 
                                            dataset, 
...
class IrscScaledMisalignedImu(IrscImu):

    class ImuParameters(IrscImu.ImuParameters):
        def __init__(self, imuConfig):
            IrscImu.ImuParameters.__init__(self, imuConfig)
            self.data = imuConfig.data
            self.data["model"] = "scale-misalignment"

        def printDetails(self, dest=sys.stdout):
            IrscImu.ImuParameters.printDetails(self, dest)
    ...
    def addDesignVariables(self, problem):
        IrscImu.addDesignVariables(self, problem)
...
class IrscScaledMisalignedSizeEffectImu(IrscScaledMisalignedImu):

    class ImuParameters(IrscScaledMisalignedImu.ImuParameters):
        def __init__(self, imuConfig):
            IrscScaledMisalignedImu.ImuParameters.__init__(self, imuConfig)
            self.data = imuConfig.data
            self.data["model"] = "scale-misalignment-size-effect"

        def printDetails(self, dest=sys.stdout):
            IrscScaledMisalignedImu.ImuParameters.printDetails(self, dest)
            print >> dest, "    rx_i [m]:"
